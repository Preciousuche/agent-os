...
class StreamConsumerStage:
    ...
    async def run(
        ...
        state = inp.state
        usage_tracker = inp.usage_tracker
        budgets_cfg = inp.budgets_cfg
        ...
        async for event in self._agent_run.run_turn(
            ...
            if usage_tracker and budgets_cfg:
                from agentos.engine.usage import BudgetExceededError

                exceeded, msg = usage_tracker.check_budget_limits(inp.session_key, budgets_cfg)
                if exceeded:
